...
```typescript
type MemoryType =
  | "discovery"       // 关于代码库/项目的发现
  | "preference"      // 用户偏好、指令、修正
  | "file_knowledge"  // 文件修改记录、关键位置
  | "general"         // 其他
```

> **精简说明**: 原始设计有 8 种类型（含 architecture/pattern/correction/decision），经评估后合并为 4 种。LLM 实际使用中难以精确区分过细的分类，4 种足以覆盖所有场景。
...
```
decayed = confidence - weeks_since_last_access × 2
```
- 线性衰减，每周 -2 分（简单直觉）
- 以最近访问时间为基准（非创建时间）
- 最低不低于 0

> **精简说明**: 原设计使用复合衰减 `confidence × 0.95^weeks` + log2 访问频率加成。经评估后简化为线性衰减 — 没有 benchmark 证明复合公式更优，线性公式更易理解和调试。

**相关性评分** (`relevanceScore`):
```
score = decayedConfidence
```
- 直接使用衰减后置信度排序，不做额外加成
...
1. **Compaction 提取** (`session/compaction.ts`):
   ...
   - 自动创建 `discovery`/`file_knowledge`/`general` 类型记忆
   - 非阻塞：提取失败不影响正常会话流
