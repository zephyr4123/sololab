...
<br/>
<p align="center"><img src="docs/images/divider.svg" width="600" /></p>
<br/>
...
<td>学术论文 Agent · 8 工具自主编排 · 流式预览 · 6 模板 · 像素级 PDF 导出</td>
<td align="center"><img src="https://img.shields.io/badge/-Ready-00C853?style=flat-square" alt="Ready"/></td>
</tr>
<tr>
...
## ✍️ WriterAI — 学术论文 Agent

> **一句话描述**：让研究者像跟合作者对话那样写论文 — 8 工具自主编排 × 流式预览 × 6 种期刊模板 × 像素级 PDF 导出
...
<td align="center" width="25%">
<b>8 个专业工具</b><br/>
<sub>搜索 / 大纲 / 流式写作 / 引用<br/>沙箱代码 / 图表 / 知识库 / 状态</sub>
</td>
<td align="center" width="25%">
<b>4 层提示词架构</b><br/>
<sub>静态 Prompt + 状态锚<br/>工具返回增强 + Agent Hooks</sub>
</td>
<td align="center" width="25%">
<b>6 种期刊模板</b><br/>
<sub>Nature / CVPR / ICCV / IEEE<br/>ACM / 中文期刊 · YAML 热插拔</sub>
</td>
<td align="center" width="25%">
<b>PDF 像素一致</b><br/>
<sub>Playwright + KaTeX<br/>数学公式 · 图表内联 · 中文字体</sub>
</td>
</tr>
</table>
</center>
...
<tr>
<td width="50%" valign="top">
独立研究者写论文时，最耗时的不是思路，而是 <b>反复的文献检索、格式调整、引用管理、图表绘制</b>。传统 AI 写作助手停留在"帮我润色这段话"的层次，无法理解论文整体结构与投稿规范。<br/><br/>

WriterAI 把完整学术写作流交给一个 <b>会用工具的 Agent</b>：自主去 arXiv/Scholar 搜索 → 按期刊模板生成大纲 → 流式写作每个章节 → 在沙箱中跑 matplotlib 出图 → 插入引用与图表 → 一键导出与预览<b>像素一致</b>的 PDF。<br/><br/>

<em>"给它一句话描述，它会先搜文献、列大纲征求同意、流式写出章节、遇到数据画图、遇到公式调 KaTeX、最后输出投稿即用的 PDF。"</em>
</td>
<td width="50%" valign="top">

<img src="docs/images/writer-concept.png" alt="传统学术写作 vs WriterAI Agent" />
...
### 🔧 8 个专业工具 · 单 Agent 自主编排

<center>
<table align="center" width="960" cellpadding="8">
<tr>
<td align="center" width="25%" valign="top">
<h4>🔍 <code>search_literature</code></h4>
<sub>arXiv + Scholar + Tavily<br/>实时检索并验证文献</sub>
</td>
<td align="center" width="25%" valign="top">
<h4>📋 <code>create_outline</code></h4>
<sub>基于 YAML 模板生成<br/>章节结构 · 类型 · 顺序</sub>
</td>
<td align="center" width="25%" valign="top">
<h4>✍️ <code>write_section</code></h4>
<sub>SSE 流式写作单个章节<br/>内联 [FIGURE:] 占位符</sub>
</td>
<td align="center" width="25%" valign="top">
<h4>📚 <code>manage_reference</code></h4>
<sub>add / remove 自动编号<br/>去重 · 跟随模板格式</sub>
</td>
</tr>
<tr>
<td align="center" width="25%" valign="top">
<h4>🐍 <code>execute_code</code></h4>
<sub>Docker 沙箱 · 无网络<br/>matplotlib / plotly / pandas</sub>
</td>
<td align="center" width="25%" valign="top">
<h4>🖼️ <code>insert_figure</code></h4>
<sub>替换正文 [FIGURE:] 占位符<br/>图表流入段落间</sub>
</td>
<td align="center" width="25%" valign="top">
<h4>📎 <code>search_knowledge</code></h4>
<sub>按 doc_id 隔离的附件检索<br/>仅作内部上下文</sub>
</td>
<td align="center" width="25%" valign="top">
<h4>📄 <code>get_document</code></h4>
<sub>读取当前文档状态<br/>做上下文决策</sub>
</td>
</tr>
</table>
</center>
...
描述一个写作任务后，Agent 自动搜索文献 → 生成大纲 → 逐章节流式写作，右侧预览实时呈现每一段新生成的文字：

<div align="center">
<img src="docs/images/writer-writing.png" width="720" alt="WriterAI 流式写作 — 左聊天 + 中预览" />
<br/>
<sub>左 Chat · 中 文档预览实时流式生成 · 工具气泡追踪 Agent 每一步决策</sub>
</div>

输入框左下角的 <b>+ 菜单</b> 可上传参考 PDF/MD/DOCX，附件严格按文档隔离（<code>project_id = "writer:{doc_id}"</code> 命名空间），不会污染其他会话的检索结果：

<div align="center">
<img src="docs/images/writer-attachments.png" width="720" alt="PlusMenu 附件管理 · 按文档隔离" />
<br/>
<sub>拖拽或点选上传 · 自动分块 + 向量化 · 状态轮询 · 删除 · 按 doc_id 严格隔离</sub>
</div>

一键导出为 PDF，数学公式通过 KaTeX 渲染，图表内联嵌入，中文字体自动识别，预览与 PDF <b>像素一致</b>：

<div align="center">
<img src="docs/images/writer-export.png" width="720" alt="WriterAI PDF 导出 · 像素一致" />
<br/>
<sub>Playwright 驱动的 headless Chromium · KaTeX 数学 · 图表 Base64 内联 · 6 模板自动切换</sub>
</div>

### 🧠 4 层提示词架构 · Soft Enforcement

AI 写作里最恼人的 bug 是 Agent "<b>抽风重写</b>" — 用户中文写到一半，Agent 突然切英文重写全文。WriterAI 没用硬状态机，而是用 **soft enforcement**：状态通过提示词注入最近 attention 窗口，让 Agent 自主修正。
...
<tr>
<td align="center"><b>L1 · 静态 Prompt</b></td>
<td>模板、角色、工具约定</td>
<td>"你是 Nature 期刊写作助手，遵循算法化 Figure 协议..."</td>
</tr>
<tr>
<td align="center"><b>L2 · 状态锚</b></td>
<td>注入到 user message 最前面，<b>最近 attention 窗口</b></td>
<td>🔒 LANGUAGE LOCK: 中文 ／ 📌 PENDING FIGURES: [fig1, fig2]</td>
</tr>
<tr>
<td align="center"><b>L3 · 工具返回增强</b></td>
<td>write_section / insert_figure 返回后追加提示</td>
<td>"你刚写的章节检测到语言为中文，后续请保持一致"</td>
</tr>
<tr>
<td align="center"><b>L4 · Agent Hooks</b></td>
<td>Python 层在工具调用后做状态提取</td>
<td>自动写入 <code>metadata_json.language_lock</code> 持久化</td>
</tr>
</table>
</center>

> **设计理念：软阻断 > 硬阻断。** 状态通过提示词流入最近注意力窗口，让 Agent 自主修正；无需维护状态机、无需硬拒绝、无需重启对话。中英切换、图表占位符漏插等问题在 v6.4 后零复现。

### 🏛️ 技术架构

<div align="center">
<img src="docs/images/writer-architecture.png" width="720" alt="WriterAI 技术架构" />
<br/>
<sub>Frontend ←→ FastAPI ←→ WriterAgent ←→ 8 Tools · Docker Sandbox · Playwright PDF Engine</sub>
</div>
...
<tr>
<td width="50%" valign="top">
<b>📦 单表 JSONB 数据模型</b><br/>
<sub><code>writer_documents</code> 表 + JSONB 存 sections / references / figures</sub><br/>
<sub><code>metadata_json</code> 存 language_lock / pending_placeholders / conversation</sub><br/>
<sub>加字段零迁移，用 <code>flag_modified</code> 持久化</sub>
<br/><br/>
<b>🔄 多轮对话持久化</b><br/>
<sub>最近 5 轮完整消息序列持久化（tool_call_id 严格配对）</sub><br/>
<sub>下一轮直接 replay，语言锁与占位符从 metadata 重建</sub><br/>
<sub>刷新浏览器不丢上下文，跨 session 可续写</sub>
</td>
<td width="50%" valign="top">
<b>🐳 Docker 代码沙箱</b><br/>
<sub><code>--network none</code> · <code>--memory 512m</code> · <code>--read-only</code></sub><br/>
<sub><code>cap-drop ALL</code> · 30s 超时 · <b>绝不注入 API 密钥</b></sub><br/>
<sub>matplotlib / plotly / pandas / numpy 预装，中日韩字体就绪</sub>
<br/><br/>
<b>📎 附件按文档隔离</b><br/>
<sub><code>project_id = "writer:{doc_id}"</code> 命名空间</sub><br/>
<sub>search_knowledge 工具强制带 scope 过滤</sub><br/>
<sub>A 文档附件不会污染 B 文档的 Agent 检索</sub>
</td>
</tr>
</table>
</center>
...
每个模板只是一个 YAML + 可选的 .docx 文件，放入 <code>modules/writer/templates/</code> 即刻生效。无需改代码、无需重启。

<center>
<table align="center" width="960" cellpadding="8">
<tr>
<td align="center" width="16%"><b>Nature</b><br/><sub>通用期刊</sub></td>
<td align="center" width="16%"><b>CVPR</b><br/><sub>双栏 · 视觉</sub></td>
<td align="center" width="16%"><b>ICCV</b><br/><sub>双栏 · 视觉</sub></td>
<td align="center" width="16%"><b>IEEE</b><br/><sub>工程学术</sub></td>
<td align="center" width="16%"><b>ACM</b><br/><sub>计算机学术</sub></td>
<td align="center" width="16%"><b>中文期刊</b><br/><sub>中文双语</sub></td>
</tr>
</table>
</center>

> 新增模板只需在 <code>modules/writer/templates/your_template/</code> 放一个 <code>template.yaml</code>（定义 sections / citation / 页面设置）和可选的 <code>template.docx</code>。<code>TemplateRegistry</code> 启动时自动发现。
...
<img src="https://img.shields.io/badge/KaTeX-329F7E?style=flat-square" alt="KaTeX"/>
<img src="https://img.shields.io/badge/Docker_Sandbox-2496ED?style=flat-square&logo=docker&logoColor=white" alt="Sandbox"/>
...
<details>
<summary><h2>🧑‍💻 开发新模块</h2></summary>

<br/>

SoloLab 采用 **manifest + ABC 抽象类** 的插件架构，同时支持通过独立引擎（如 CodeLab 的 OpenCode）集成高度专业化的模块。新模块不需要修改任何核心代码，不需要注册路由，不需要手动接线——放入目录即刻生效。
...
│   ├── manifest.json
│   ├── module.py
│   ├── agent.py        # WriterAgent 编排器
│   ├── tools/          # 8 专业工具
│   ├── templates/      # YAML 模板 (Nature/CVPR/...)
│   ├── sandbox/        # Docker 代码沙箱
│   └── export/         # PDF 导出 (Playwright + KaTeX)
└── your-module/        # 🆕 你的新模块
    ├── manifest.json
    └── module.py
```
...
**引擎型模块**（如 CodeLab）也遵循同样的模式 — 通过 `api/codelab.py` 代理层接入，Backend 负责 Session 统一管理，前端通过标准 API 调用。平台对模块的集成方式保持开放：既支持原生 Python 模块，也支持独立引擎通过 HTTP 桥接。
...
**License** · Apache License 2.0 &nbsp;|&nbsp; **CodeLab Engine** · Based on [OpenCode](https://github.com/nicepkg/opencode) (MIT License)

<br/>

<b>Author</b> · Suxiang Huang &nbsp;·&nbsp; 📧 huangsuxiang5@gmail.com &nbsp;·&nbsp; 💬 WeChat: 13976457218
<br/>
<sub>🏛️ Memory and Reasoning Lab, Tianjin University</sub>
...
<img src="logo.png" width="36" alt="SoloLab" />
<br/>
<sub>
<b>SoloLab</b> · <i>One-Person Lab, Infinite Minds</i><br/><br/>
